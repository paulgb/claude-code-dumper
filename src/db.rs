...
impl ClaudeDatabase {
    ...
    pub fn get_conversation_summaries(&self) -> Result<Vec<(String, String, i64, String)>> {
        let mut stmt = self.conn.prepare("SELECT leaf_uuid, summary FROM conversation_summaries")?;
    ...
    pub fn get_conversation(&self, leaf: &str) -> Result<Vec<Message>> {
        ...
        while let Some(id) = current_id {
            ...
            let (parent_uuid, session_id, base_timestamp, message_type, cwd, user_type, version, is_sidechain) = base_row;
            ...
            if message_type == "user" {
                ...
                let (cost_usd, duration_ms, message, is_api_error_message, timestamp, model) = assistant_row;
